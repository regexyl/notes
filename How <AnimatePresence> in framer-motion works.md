...
An intuitive, non-Reacty way is to (1) start the exit animation, (2) wait till the animation is done, then (3) remove the element from the DOM. That's too much of a chore, isn't it? It's probably also not declarative enough for our tastes, so perhaps this is where external libraries are able to step in and create a workaround.
...
Did someone say "give me a tool that reliably stores existing state during re-renders *and* allows for control over fine-grained rendering (aka stripping some of that power away from React and delegating it to ourselves)"? `React.useRef` 